...
DEMO SCRIPT: This script tells the end-to-end story of the project.
            (V2 - Uses fully qualified names to be session-independent)
==================================================================================
*/
...
-- -------------------------------------------------------------------------------
SELECT * FROM CGM_HEALTH.ANALYTICS.LANDING_LIBRE_RAW LIMIT 10;
...
-- -------------------------------------------------------------------------------
SELECT * FROM CGM_HEALTH.ANALYTICS.RAW_READINGS LIMIT 10;
...
-- -------------------------------------------------------------------------------
SELECT * FROM CGM_HEALTH.ANALYTICS.CLASSIFIED_NOTES LIMIT 10;

-- Show the "before and after" of the NLP
SELECT
    r.NOTES,
    c.LIFESTYLE_CATEGORY
FROM CGM_HEALTH.ANALYTICS.RAW_READINGS r
JOIN CGM_HEALTH.ANALYTICS.CLASSIFIED_NOTES c ON r.TIMESTAMP = c.TIMESTAMP
...
SELECT 
    ...
    GLYCATION_INDEX 
FROM 
    CGM_HEALTH.ANALYTICS.GLUCOSE_FEATURES
WHERE 
    GLYCATION_INDEX > 0
ORDER BY 
...
SELECT
    ...
    ANOMALY_SCORE
FROM
    CGM_HEALTH.ANALYTICS.GLUCOSE_ANOMALIES
WHERE
